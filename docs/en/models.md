...
FP16 keeps graph inputs and outputs in FP32 and preserves sensitive operators such as Resize in FP32. `auto` prefers FP16 when WebGPU exposes `shader-f16`; otherwise it selects FP32. Manual `fp16` selection should normally allow fallback unless strict failure is a product requirement.
