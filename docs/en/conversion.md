...
The current root model keeps the WebGPU-validated FP32 artifact. Version values such as `1.0.1` are provenance labels in reports, not model directory names. The FP32 artifact passed CPU parity on seven licensed fixtures, strict browser WASM, and a physical WebGPU adapter with no fallback. The FP16 artifact is byte-identical to the previously accepted artifact and passed CPU precision acceptance plus real browser WASM and WebGPU validation.

Reproduce the sanitized graph from an externally exported original FP32 artifact into the current root model:

```powershell
.\.venv-model\Scripts\python.exe tools/model-pipeline/ppdoclayout/sanitize_fp32.py `
  --source <path-to-original-model-fp32.onnx> `
  --output models/pp-doclayoutv3/model-fp32.onnx
```

Then run the FP32 parity, browser evidence, variant validation, and manifest commands with the current reports under `tools/model-pipeline/reports/`; the manifest generator writes `models/pp-doclayoutv3/manifest.json` and rejects any hash, fixture, provider, precision, or fallback mismatch.
