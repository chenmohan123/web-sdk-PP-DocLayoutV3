...
模型 `1.0.1` 准备发布到不可覆盖的 `v1.0.1-models` release。FP32 在 7 张授权图片上通过 CPU parity、严格浏览器 WASM 和物理 WebGPU，且没有 fallback。FP16 与已验收的 `1.0.0` FP16 文件字节完全一致。历史 `v1.0.0-models` 资产保持不变。
