...
FP32 验收覆盖 7 张授权测试图，在阈值 0.5 下检测数量、标签序列和阅读顺序全部一致，并通过浏览器 WASM 运行。FP16 验收覆盖同一集合并通过真实 WebGPU 运行。FP32 尚未记录物理 WebGPU 验证，因此默认清单仅把 FP32 用于 WASM。INT8 候选未达到检测匹配阈值，因此没有发布。
