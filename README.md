...
当前模型的支持矩阵为：WebGPU 和 CPU/WASM 均支持 FP16，WebGPU 和 CPU/WASM 均支持已验证的 FP32。全自动模式在具备 `shader-f16` 时优先 WebGPU FP16；没有该能力时可使用 WebGPU FP32，运行时失败后再尝试 WASM/CPU FP16 和 FP32。使用默认模型时，SDK 会以 `CAPABILITY_UNSUPPORTED` 拒绝清单中不存在的显式组合；`allowFallback` 只会尝试清单中的有效候选。在线 Demo 仅在“自动后端 + 自动精度”时允许运行时回退；手动选择任一后端或精度后会严格执行。自定义清单若包含已通过验证的其他组合，SDK 和 Demo 仍允许对应组合。

当前模型：FP16 为 74,279,796 字节，FP32 为 142,574,928 字节；两个文件与 manifest 位于 `models/pp-doclayoutv3/` 根目录，由 Git LFS 管理并在更新时一起替换。FP16 通过 CPU 精度验收、真实浏览器 WASM 和真实 WebGPU 验证；FP32 已通过 7 张授权图片的数值对齐、浏览器 WASM 和物理 WebGPU 验证。上游模型是 float32，不支持 FP64 推理。模型使用 Apache-2.0，来源为 PaddlePaddle `PP-DocLayoutV3_safetensors`，详见 [模型文档](docs/zh-CN/models.md) 与 [第三方声明](THIRD_PARTY_NOTICES.md)。历史版本只保留在验证报告和来源 revision 中。
