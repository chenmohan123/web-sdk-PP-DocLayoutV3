...
The default `1.0.1` support matrix includes validated WebGPU FP16 and FP32, plus CPU/WASM FP32. Fully automatic selection prefers WebGPU FP16 when `shader-f16` is available; without `shader-f16` it can use WebGPU FP32, then fall back to WASM/CPU FP32 if runtime creation fails. The SDK rejects explicit pairs absent from the manifest with `CAPABILITY_UNSUPPORTED`; `allowFallback` only tries valid manifest candidates. The live Demo enables runtime fallback only for Auto backend + Auto precision. Any manual backend or precision choice is strict: CPU still does not support FP16, and changing backend corrects an invalid precision with a notice. Custom manifests with separately validated variants remain supported by both the SDK and Demo.

The default `1.0.1` model is 74,279,796 bytes for FP16 and 142,574,928 bytes for FP32. FP16 physical WebGPU evidence covers a single sample. FP32 is about twice the size of FP16, may be slower, and uses more GPU memory; it passed seven licensed fixtures in numerical parity, strict browser WASM, and physical WebGPU validation. The upstream model is float32, not FP64, and FP64 inference is unsupported. The PaddlePaddle `PP-DocLayoutV3_safetensors` model is Apache-2.0; see [Models](docs/en/models.md) and [Third-Party Notices](THIRD_PARTY_NOTICES.md). Historical model `1.0.0` and SDK `1.0.4` assets remain unchanged.
