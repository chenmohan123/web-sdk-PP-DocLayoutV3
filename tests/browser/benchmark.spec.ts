...
test("records strict seven-fixture browser evidence", async ({ browser, page }) => {
  ...
  const fixtureEvidence = result.fixtures.map(({ detections, ...fixture }) => {
    ...
    expect(parityMetrics.iou).toBeGreaterThanOrEqual(parityThresholds.iou);
    expect(parityMetrics.maxScoreDelta).toBeLessThanOrEqual(parityThresholds.maxScoreDelta);
    expect(parityMetrics.meanPolygonPointDistancePixels).toBeLessThanOrEqual(
      parityThresholds.meanPolygonPointDistancePixels
    );
    return { ...fixture, parityMetrics, parityThresholds };
  });
