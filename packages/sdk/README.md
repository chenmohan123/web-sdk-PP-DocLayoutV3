...
- 当前模型根目录中的 FP16 和 FP32 变体均支持 WebGPU 和 WASM/CPU；清单中的 `model.version` 仅用于元数据，模型文件不再按版本建立目录。
...
- The current root model supports FP16 and FP32 on both WebGPU and WASM/CPU; `model.version` remains manifest metadata and is not used as a model directory name.
